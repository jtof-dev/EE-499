...
Finally, the results from SingLEM and the PSS equation are checked for normality, and evaulated using either a One-Way ANOVA (for normal and equally distributed data) or a Kruskal-Wallis test (for non-parametric data).

\section{Analysis}
Upon fine-tuning SingLEM, we got a validation accuracy of 73.7\% after six epochs\cite{jtof-dev-EE-499-2026}. This top accuracy was reached after significant difficulties with over- and under-fitting to the dataset. While this result does not appear to suffer from either of those issues, it is not clear that this is a good fine-tune.
...
\begin{figure*}[!p]
    \centering
    \small % Makes all tables in this figure slightly smaller to save vertical space
    
    % --- TABLE 1 ---
    \captionof{table}{Behavioral Performance Metrics by Task and Auditory Condition (Participant: Andy)}
    ...
    % --- TABLE 2 ---
    \captionof{table}{Statistical Analysis of Performance and EEG Metrics (Stroop Task)}
    ...
    % --- TABLE 3 ---
    \captionof{table}{Statistical Analysis of Performance and EEG Metrics (Typing Task)}
    ...
    \vspace{0.5cm} % Minimal gap before image

    % --- IMAGE (Now utilizing the saved space) ---
    \begin{minipage}{0.8\textwidth} % Increased width from 0.48 to 0.8
        \centering
        \includegraphics[width=\linewidth]{figures/singlem_stroop_anxiety_over_time.png}
        \caption{Anxiety over time during the Stroop Test.}
        \label{fig:my_first_pic}
    \end{minipage}
\end{figure*}
